...
会話的なWeb UIを実装するためにReactを使うしかないのかなと思っていた。そのノウハウを教えてくれる本も読んだ。しかしわたしはReactに気乗りがしなかった。わたしが目標とするwebサイトにReactはあまり適合的でなかった。もっと軽い方法はないものか？と迷っていた。そうこうするうち２０２６年３月、Web記事 link:https://gihyo.jp/article/2026/02/htmx-introduction[gihyo.jp 「HTMLを拡張し⁠⁠、JSなしで動的UIを作るhtmx」, 嶌田喬行（しまだたかゆき）] を読んだ。この記事の冒頭にhtmxを使ったwebページのサンプルが紹介されていた。
